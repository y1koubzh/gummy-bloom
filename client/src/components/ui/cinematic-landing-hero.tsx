...
export function CinematicHero({ 
  brandName = "Gummy Bloom",
  tagline1 = "Experience Health,",
  tagline2 = "like never before.",
  cardHeading = "Innovation, redefined.",
  cardDescription = <><span className="text-white font-semibold">Gummy Bloom</span> offers the most advanced customized supplements, tailored exactly to your nutritional needs through scientific assessment.</>,
  metricValue = 100,
  metricLabel = "Purity %",
  ctaHeading = "Start your Bloom.",
  ctaDescription = "Join thousands of satisfied customers and start your journey towards personalized health today.",
  className, 
  ...props 
}: CinematicHeroProps) {
  ...
  // 2. Auto-playing Cinematic Sequence (Video-like Experience)
  useEffect(() => {
    if (typeof window === "undefined") return;
    ...
    const ctx = gsap.context(() => {
      ...
      // Create a single master timeline for the "Video" experience
      const videoTl = gsap.timeline({ delay: 0.5 });
      ...
      videoTl
        ...
        .to({}, { duration: 1 }) // Hold text for a second

        // Phase 2: Card Entrance (Pulls background text back)
        .to([".hero-text-wrapper", ".bg-grid-theme"], { scale: 1.15, filter: "blur(20px)", opacity: 0.2, ease: "power2.inOut", duration: 2 })
        .to(".main-card", { y: 0, ease: "power3.inOut", duration: 1.8 }, "-=1.5")
        
        // Phase 3: Transition to full screen and bring in phone
        .to(".main-card", { width: "100%", height: "100%", borderRadius: "0px", ease: "power3.inOut", duration: 1.5 })
        .fromTo(".mockup-scroll-wrapper",
          { y: 300, z: -500, rotationX: 50, rotationY: -30, autoAlpha: 0, scale: 0.6 },
          { y: 0, z: 0, rotationX: 0, rotationY: 0, autoAlpha: 1, scale: 1, ease: "expo.out", duration: 2.2 }, "-=0.5"
        )
        // Animate phone widgets and progress
        .fromTo(".phone-widget", { y: 40, autoAlpha: 0, scale: 0.95 }, { y: 0, autoAlpha: 1, scale: 1, stagger: 0.15, ease: "back.out(1.2)", duration: 1.2 }, "-=1.2")
        .to(".progress-ring", { strokeDashoffset: 60, duration: 2.5, ease: "power3.inOut" }, "-=1.0")
        .to(".counter-val", { innerHTML: metricValue, snap: { innerHTML: 1 }, duration: 2, ease: "expo.out" }, "-=2.2")
        
        // Phase 4: Floating badges and side text
        .fromTo(".floating-badge", { y: 80, autoAlpha: 0, scale: 0.7, rotationZ: -10 }, { y: 0, autoAlpha: 1, scale: 1, rotationZ: 0, ease: "back.out(1.5)", duration: 1.2, stagger: 0.2 }, "-=1.8")
        .fromTo(".card-left-text", { x: -50, autoAlpha: 0 }, { x: 0, autoAlpha: 1, ease: "power4.out", duration: 1.5 }, "-=1.2")
        .fromTo(".card-right-text", { x: 50, autoAlpha: 0, scale: 0.8 }, { x: 0, autoAlpha: 1, scale: 1, ease: "expo.out", duration: 1.5 }, "<")
        
        .to({}, { duration: 3 }) // Cinematic pause to admire the dashboard

        // Phase 5: Reveal CTA (Pullback to final state)
        ...
        >
          ...
          <div className="relative w-full h-full max-w-7xl mx-auto px-4 lg:px-12 flex flex-col justify-evenly lg:grid lg:grid-cols-3 items-center lg:gap-8 z-10 py-6 lg:py-0">
            ...
            <div className="card-right-text gsap-reveal order-1 lg:order-3 flex justify-center lg:justify-end z-20 w-full">
              <h2 className="text-6xl md:text-[6rem] lg:text-[8rem] font-black uppercase tracking-tighter text-card-silver-matte lg:mt-0 leading-tight">
                {brandName}
              </h2>
            </div>
            ...
            <div className="mockup-scroll-wrapper order-2 lg:order-2 relative w-full h-[380px] lg:h-[600px] flex items-center justify-center z-10" style={{ perspective: "1000px" }}>
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                    ...
                    <div className="relative w-full h-full pt-12 px-5 pb-8 flex flex-col">
                      <div className="phone-widget flex justify-between items-center mb-8">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xl font-bold tracking-tight text-white drop-shadow-md">Health</span>
                        </div>
                        <div className="w-9 h-9 rounded-full bg-white/5 text-neutral-200 flex items-center justify-center font-bold text-sm border border-white/10 shadow-lg shadow-black/50">GB</div>
                      </div>
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Eco-Pack</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">Sustainably Made</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Lab Verified</p>
                    <p className="text-blue-200/50 text-[10px] lg:text-xs font-medium">100% Purity</p>
                  </div>
                </div>
